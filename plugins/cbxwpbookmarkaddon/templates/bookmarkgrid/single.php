<?php
	...
	if ( $show_thumb ) {
		...
		if ( function_exists( 'fifu_main_image_url' ) && $pro_grid_thumb_size == 'fifu' ) {
			//$img_url = get_post_meta($object_id, 'fifu_image_url_' . $att_id, true);
		...
		$thumb_html .= '<a class="cbxbookmark_card_teaser" href="' . get_permalink( $object_id ) . '"><img src="' . apply_filters('cbxwpbookmark_bookmarkgrid_single_thumb_url', $img_url, $object_id, $object_type) . '" alt="bookmarkimage"  /></a>';
	}
	...
	$card_cols_class =  apply_filters('cbxwpbookmark_bookmarkgrid_cols_class', 'col-12 col-xs-12 col-sm-6 col-md-4', 'single');

	echo '<div class="'.esc_attr($card_cols_class).' cbxbookmark_card_col"><div class="cbxbookmark_card">';

	do_action( 'cbxwpbookmark_bookmarkgrid_single_item_start', $object_id, $item );

	echo $thumb_html;
	echo '<div class="cbxbookmark_card_clear"></div>
								<div class="cbxbookmark_card_container">
									<p class="cbxbookmark_card_title"><a href="' . get_permalink( $object_id ) . '">' . apply_filters('cbxwpbookmark_bookmarkgrid_single_title', wp_strip_all_tags( get_the_title( $object_id ) ), $object_id, $object_type) . '</a></p>
									<div class="cbxbookmark_card_clear"></div>' .

	     ( ( $cat_title != '' || $delete_html != '' ) ? '<p class="cbxbookmark_card_cat_delete">' . wp_unslash( $cat_title ) . $delete_html . '</p>' : '' ) . $track_input_output . '
								</div>';

	do_action( 'cbxwpbookmark_bookmarkgrid_single_item_end', $object_id, $item );
