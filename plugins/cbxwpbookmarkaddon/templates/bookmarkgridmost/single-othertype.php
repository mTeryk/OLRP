	...
	if ( $enable_buddypress_bookmark && $object_type == 'buddypress_activity' && function_exists( 'bp_activity_get' ) ) {
		...
		if ( isset( $activity_get['activities'][0] ) ) {
			...
			if ( $show_thumb ) {
				...
				$thumb_html .= '<a class="cbxbookmark_card_teaser" href="' . bp_activity_get_permalink( $object_id ) . '"><img src="' . $img_url . '" alt="bookmarkimage"  /></a>';
			}

			$card_cols_class =  apply_filters('cbxwpbookmark_bookmarkgridmost_cols_class', 'col-xs-12 col-sm-6 col-md-4', 'othertype');

			echo '<div class="'.esc_attr($card_cols_class).' cbxbookmark_card_col"><div class="cbxbookmark_card">';

			do_action( 'cbxwpbookmark_bookmarkgridmost_othertype_item_start', $object_id, $item );

			echo $thumb_html;
			echo '<div class="cbxbookmark_card_clear"></div>
								<div class="cbxbookmark_card_container">
									<p class="cbxbookmark_card_title"><a href="' . bp_activity_get_permalink( $object_id ) . '">' . wp_strip_all_tags( $activity->content ) . '</a></p>
									<div class="cbxbookmark_card_clear"></div>
								</div>';

			do_action( 'cbxwpbookmark_bookmarkgridmost_othertype_item_end', $object_id, $item );
