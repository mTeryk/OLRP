...
class CBXWPBookmark_Public {
	...
	public function find_category() {
		...
		if ( $cats_by_user != null ) {
			...
			$message['code'] = 0;
			$message['msg']  = esc_html__( 'Resource List not found, create one.', 'cbxwpbookmark' );
		}
	...
	public function bookmark_mycat_shortcode( $attr ) {
		...
		if ( $bookmark_mode != 'no_cat' ) {
			...
			$output .= __( '<strong>Sorry, This widget is not compatible as per setting. This widget can be used only if bookmark mode is "User owns category" or "Global Resource List"</strong>', 'cbxwpbookmark' );
		}
	...
	public function add_category_std() {
		...
		if ( $bookmark_mode != 'user_cat' ) {
			$message['code'] = 0;
			$message['msg']  = esc_html__( 'Resource List add failed!', 'cbxwpbookmark' );
			echo json_encode( $message );
		...
		if ( $cat_name == '' ) {
			$message['code'] = 0;
			$message['msg']  = esc_html__( 'Resource List name can not be empty', 'cbxwpbookmark' );
		} else if ( intval( $duplicate ) > 0 ) {
			$message['code'] = 0;
			$message['msg']  = esc_html__( 'Resource List with same name already exists!', 'cbxwpbookmark' );
		} else {
			//create category
			...
			if ( $return !== false ) {
				...
				$message['code']      = 1;
				$message['msg']       = esc_html__( 'Resource List created successfully!', 'cbxwpbookmark' );
				$message['id']        = $cat_id;
	...
	public function add_category() {
		...
		if ( $cat_name == '' ) {
			...
			if ( $return !== false ) {
				...
				$message['code']   = 1;
				$message['msg']    = esc_html__( 'Resource List created/edited successfully!', 'cbxwpbookmark' );
				...
				$message['code'] = 0;
				$message['msg']  = esc_html__( 'Resource List add/edit failed!', 'cbxwpbookmark' );
			}
	...
	public function edit_category() {
		...
		if ( $cat_name == '' ) {
			...
			$message['code'] = 0;
			$message['msg']  = esc_html__( 'Resource List id missing, are you cheating?', 'cbxwpbookmark' );
		} else if ( intval( $duplicate ) > 0 ) {
			$message['code'] = 0;
			$message['msg']  = esc_html__( 'Another Resource List with same name already exists!', 'cbxwpbookmark' );
		} else {
			// Update Query
			$return = $wpdb->update(
				$category_table, array(
			...
			if ( $return !== false ) {
				...
				$message['code']   = 1;
				$message['msg']    = esc_html__( 'Resource List updated successfully!', 'cbxwpbookmark' );
				$message['cat_id'] = $cat_id;
	...
	public function enqueue_scripts() {
		...
		$category_template = '
            <div class="cbxbookmark-mycat-editbox">
                <input class="cbxbmedit-catname cbxbmedit-catname-edit" name="catname" value="##catname##" placeholder="' . esc_html__( 'Resource List title', 'cbxwpbookmark' ) . '" />                
                <select class="cbxbmedit-privacy input-catprivacy ' . $cat_hide_class . '" name="catprivacy">
                  <option value="1" title="' . esc_html__( 'Public Resource List', 'cbxwpbookmark' ) . '">' . esc_html__( 'Public', 'cbxwpbookmark' ) . '</option>
                  <option value="0" title="' . esc_html__( 'Private Resource List', 'cbxwpbookmark' ) . '">' . esc_html__( 'Private', 'cbxwpbookmark' ) . '</option>
                </select>
		...
		$cbxwpbookmark_translation = array(
			...
			'category_delete_success'    => esc_html__( 'Resource List deleted successfully', 'cbxwpbookmark' ),
			'category_delete_error'      => esc_html__( 'Unable to delete the category', 'cbxwpbookmark' ),
			'areyousuretodeletecat'      => esc_html__( 'Are you sure you want to delete this Bookmark Resource List?', 'cbxwpbookmark' ),
			...
			'category_name_empty'        => esc_html__( 'Resource List name can not be empty', 'cbxwpbookmark' ),
			'add_to_head_default'        => esc_html__( 'Click Resource List to Bookmark', 'cbxwpbookmark' ),
			'add_to_head_cat_list'       => esc_html__( 'Click to Edit Resource List', 'cbxwpbookmark' ),
			'add_to_head_cat_edit'       => esc_html__( 'Edit Resource List', 'cbxwpbookmark' ),
			'add_to_head_cat_create'     => esc_html__( 'Create Resource List', 'cbxwpbookmark' ),
			'add_to_head_max_cat'        => esc_html__( 'Maximum resource list limit reached', 'cbxwpbookmark' ),
			//'category_loaded_add'        => esc_html__('Click Resource List to Bookmark', 'cbxwpbookmark'),
			'max_cat_limit'              => 0,
			'max_cat_limit_error'        => esc_html__( 'Sorry, you reached the maximum resource list limit and to create one one, please delete unnecessary categories first', 'cbxwpbookmark' ),
